...
class TSP_QAOA:
    ...
    def execute_circuit(self, theta):
        """
        Simulate the circuit using Qiskit's built-in Statevector simulator.
        Compatible with Qiskit 2.x (no qiskit-aer required).
        ...
        # Decompose PauliEvolutionGate into primitive gates for Statevector simulation
        qc_decomposed = transpile(
            qc,
            basis_gates=["cx", "u", "p", "h", "rx", "ry", "rz", "x"],
            optimization_level=0,
        )
        state = Statevector(qc_decomposed)
        return state
